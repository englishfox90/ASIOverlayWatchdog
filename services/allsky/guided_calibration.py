...
def calibrate_from_anchors(
    ...
    Args:
        anchors: list of (pixel_x, pixel_y, ra_deg, dec_deg) — one per clicked
            and named star. RA/Dec are J2000-of-catalog (same frame the rest of
            the pipeline uses).
    ...
    for i, (_x, _y, ra, dec) in enumerate(anchors):
        alt, az = radec_to_altaz(float(ra), float(dec), lat_deg, lon_deg, dt)
        alts[i], azs[i] = float(alt), float(az)
    if np.any(alts <= 0):
        raise CalibrationError(
            "One or more identified stars are below the horizon at the capture "
            "time — check the star identifications and the system clock."
        )
    ...
    best = _coarse_search(alts, azs, px, cx, cy, a1_seed)
    model, rms = _refine(alts, azs, px, cx, cy, a1_seed, best)

    if rms > rms_limit:
        raise CalibrationError(
            f"Guided calibration RMS {rms:.1f}px exceeds limit {rms_limit:.0f}px "
            f"over {len(anchors)} anchors — re-check the star identifications "
            "(a mislabelled or mis-clicked star throws the whole fit off)."
        )
            ...
            for axis_az in ORIENT_AXIS_AZ:
                for roll_deg in ORIENT_ROLL_DEG:
                    ...
                    if best is None or err < best[0]:
                        best = (err, east_left, float(axis_alt),
                                float(axis_az), float(roll_deg))
    return best


def _refine(alts, azs, px, cx, cy, a1_seed, best):
    ...
    def residuals(p):
        ...
        m = _params_to_model(
            [cx, cy, a1, a3, a5, roll, axis_alt, axis_az], east_left)
    ...
    result = _least_squares(
        residuals, p0,
        bounds=(
            [50.0,  A3_MIN, -1500.0, -np.pi, 60.0, -180.0],
            [2000.0, A3_MAX,  500.0,  np.pi, 90.0,  540.0],
        ),
        method='trf', max_nfev=8000, ftol=1e-6,
    )
    a1, a3, a5, roll, axis_alt, axis_az = result.x
    model = _params_to_model(
        [cx, cy, a1, a3, a5, roll, axis_alt, axis_az % 360.0], east_left)
